...
LLM_API_KEY = os.getenv("LLM_API_KEY", "lm-studio") # Default for local
...
def load_prompt_template(prompt_path: str, logger=None) -> str:
    try:
...
def fetch_master_imdb_id(
    ...
    if is_tmdb_id and title_or_tmdb_id:
        try:
            ...
            if TMDB_API_KEY:
                ...
                imdb_id = tmdb_api.get_imdb_id_from_tmdb_details(TMDB_API_KEY, tmdb_id_int, str(title_or_tmdb_id), logger) # Pass logger
    ...
    if OMDB_API_KEY and title_str and valid_year_for_api:
        ...
        imdb_id = omdb_api.get_imdb_id_from_omdb(OMDB_API_KEY, title_str, valid_year_for_api) # omdb_api doesn't take logger
        if imdb_id: return imdb_id
    ...
    if TMDB_API_KEY and title_str and valid_year_for_api:
        ...
        tmdb_id_from_search, _ = tmdb_api.search_tmdb_for_movie_id(TMDB_API_KEY, title_str, valid_year_for_api, logger) # Pass logger
        if tmdb_id_from_search:
            imdb_id = tmdb_api.get_imdb_id_from_tmdb_details(TMDB_API_KEY, tmdb_id_from_search, title_str, logger) # Pass logger
            if imdb_id: return imdb_id

    if OMDB_API_KEY and title_str:
        logger.debug(f"{log_prefix} Attempting OMDB with title '{title_str}' only.")
        imdb_id = omdb_api.get_imdb_id_from_omdb(OMDB_API_KEY, title_str, None, logger) # Pass logger if omdb_api is updated
        if imdb_id: return imdb_id

    if TMDB_API_KEY and title_str:
        logger.debug(f"{log_prefix} Attempting TMDB search for '{title_str}' only, then details.")
        tmdb_id_from_search_title_only, _ = tmdb_api.search_tmdb_for_movie_id(TMDB_API_KEY, title_str, None, logger) # Pass logger
        if tmdb_id_from_search_title_only:
            imdb_id = tmdb_api.get_imdb_id_from_tmdb_details(TMDB_API_KEY, tmdb_id_from_search_title_only, title_str, logger) # Pass logger
            if imdb_id: return imdb_id
...
def run_enrichment_pipeline():
    ...
    logger.info(f"Using LLM Model: {app_config['default_llm_model_id']}")
    ...
    # LLM keys check removed as local LLM might not need it. Base URL is key.
    if not LLM_BASE_URL:
        logger.critical("LLM_BASE_URL not set. LLM calls will fail. Exiting.")
        return
    ...
    try:
        ...
        logger.info(f"OpenAI LLM client initialized for base URL: {LLM_BASE_URL}")
    except Exception as e:
        logger.critical(f"Failed to initialize OpenAI LLM client: {e}. Exiting.")
        return
    ...
    prompt_call1_template = load_prompt_template("prompts/movie_initial_data_prompt.txt", logger)
    prompt_call2_template = load_prompt_template("prompts/movie_enrich_chars_relationships_prompt.txt", logger)
    prompt_call3_template = load_prompt_template("prompts/movie_analytical_data_prompt.txt", logger)
    ...
    update_all_active_fields_for_existing = not keys_to_update_cfg
    ...
    while (new_movies_added_this_session < app_config['num_new_movies_to_fetch_this_session'] or \
        ...
        if not tmdb_page_data_raw or not tmdb_page_data_raw.get("results"):
            ...
            if not tmdb_page_data_raw or current_tmdb_page >= tmdb_page_data_raw.get("total_pages", current_tmdb_page):
        ...
        for tmdb_movie_raw_dict in movies_on_this_page_raw:
            ...
            if active_enrichers_cfg.get('initial_data'):
                if is_updating_existing_movie and not any(k in keys_to_update_cfg for k in key_to_enricher_group_map if key_to_enricher_group_map[k] == 'initial_data') and not update_all_active_fields_for_existing:
                    logger.info(f"  Skipping Initial Data regeneration for '{movie_title_for_calls}' (update existing, not in keys_to_update).")
                else:
                    ...
                    llm1_data_generated = movie_data_enricher.generate_initial_movie_data(
                        llm_client, app_config['default_llm_model_id'],
                    ...
                    if llm1_data_generated:
                        ...
                        # Transform string related movie titles from LLM1 to RelatedMovie dicts
                        update_dict = {}
                        for key, value in llm1_data_generated.model_dump(exclude={"movie_title", "movie_year"}, exclude_none=False).items():
                            if key in ["sequel", "prequel", "spin_off_of", "spin_off", "remake_of", "remake"]:
                                if isinstance(value, str) and value.strip(): # LLM might return just a title string
                                    update_dict[key] = RelatedMovie(title=value.strip()).model_dump()
                                elif isinstance(value, dict): # LLM might return a dict (ideal if it followed RelatedMovie structure)
                                    try:
                                        update_dict[key] = RelatedMovie.model_validate(value).model_dump()
                                    except Exception: # PydanticValidationError
                                        if value.get("title"): # if it's a dict but not full RelatedMovie, try to salvage title
                                             update_dict[key] = RelatedMovie(title=str(value["title"])).model_dump()
                                        else:
                                            update_dict[key] = None # or {} if MovieEntry expects dict
                                elif value is None:
                                    update_dict[key] = None
                            else:
                                update_dict[key] = value
                        working_data_dict.update(update_dict)
                    else:
            ...
            if active_enrichers_cfg.get('characters_and_relations'):
                if is_updating_existing_movie and not any(k in keys_to_update_cfg for k in key_to_enricher_group_map if key_to_enricher_group_map[k] == 'characters_and_relations') and not update_all_active_fields_for_existing:
                     logger.info(f"  Skipping Characters/Relations regeneration for '{movie_title_for_calls}' (update existing, not in keys_to_update).")
                else:
                    ...
                    if raw_chars_data:
                        raw_chars_yaml_for_prompt = yaml.dump( # LLM2 prompt expects YAML string
                        ...
                        llm2_output = character_enricher.enrich_characters_and_get_relationships(
                            llm_client, app_config['default_llm_model_id'], movie_title_for_calls, movie_year_for_calls,
                        ...
                        if llm2_output and llm2_output.character_list:
                            ...
                            temp_char_list = llm2_output.character_list
                            if active_enrichers_cfg.get('fetch_character_images'):
                                logger.info(f"    Fetching character images for '{movie_title_for_calls}'")
                                temp_char_list = character_enricher.fetch_and_assign_character_images(
                                    llm2_output.character_list, app_config['character_image_save_path'],
                            ...
                            working_data_dict["character_list"] = [char.model_dump() for char in temp_char_list] # Store as list of dicts
                            working_data_dict["relationships"] = [rel.model_dump() for rel in character_enricher.deduplicate_and_normalize_relationships(
                                temp_char_list, llm2_output.relationships, logger
            ...
            if active_enrichers_cfg.get('analytical_data'):
                if is_updating_existing_movie and not any(k in keys_to_update_cfg for k in key_to_enricher_group_map if key_to_enricher_group_map[k] == 'analytical_data') and not update_all_active_fields_for_existing:
                    logger.info(f"  Skipping Analytical Data regeneration for '{movie_title_for_calls}' (update existing, not in keys_to_update).")
                else:
                    ...
                    llm3_output = analytical_enricher.generate_analytical_data(
                        llm_client, app_config['default_llm_model_id'], movie_title_for_calls, movie_year_for_calls,
                        prompt_call3_template, max_tokens_c3, app_config, logger
                    )
                    if llm3_output:
                        logger.info(f"  Success: Analytical Data for '{movie_title_for_calls}'.")
                        working_data_dict.update(llm3_output.model_dump(exclude_none=False))
                    else:
                        logger.warning(f"  Failure or incomplete: Analytical Data for '{movie_title_for_calls}'.")
                        # Do not continue for new movie if analytical data fails, as it leads to missing fields for MovieEntry
            ...
            if active_enrichers_cfg.get('fetch_imdb_ids'):
                if is_updating_existing_movie and not ("imdb_id" in keys_to_update_cfg or \
                    ...
                    logger.info(f"  Skipping IMDb ID fetching for '{movie_title_for_calls}' (update existing, relevant keys not in keys_to_update).")
                else:
                    ...
                    for rel_key in ["sequel", "prequel", "spin_off_of", "spin_off", "remake_of", "remake"]:
                        ...
                        # Related movie can be a dict (from previous processing or llm1 if it's structured) or string (from llm1)
                        if related_movie_val and (not is_updating_existing_movie or should_update_field(rel_key)):
                            ...
                            if isinstance(related_movie_val, dict) and related_movie_val.get("title"):
                                title_to_search = related_movie_val["title"]
                                # year_hint_for_related = related_movie_val.get("year") # if RelatedMovie had year
                            elif isinstance(related_movie_val, str) and related_movie_val.strip(): # From LLM1 if it only gave a string
                                title_to_search = related_movie_val.strip()
                                # Convert this string entry into the expected dict structure for MovieEntry
                            ...
                            if title_to_search:
                                ...
                                if isinstance(working_data_dict[rel_key], dict): # Ensure it's a dict before assigning imdb_id
                                     working_data_dict[rel_key]["imdb_id"] = related_imdb_id
                                else: # Should have been converted above if string
                                     logger.warning(f"Could not set IMDb ID for {rel_key} '{title_to_search}' as its structure in working_data_dict is not a dict.")


                    if working_data_dict.get("recommendations") and isinstance(working_data_dict["recommendations"], list):
                        recs_list = working_data_dict["recommendations"] # This is already a list of dicts from LLM3 if successful
                        for rec_dict in recs_list:
                             if isinstance(rec_dict, dict) and rec_dict.get("title") and (not is_updating_existing_movie or should_update_field("recommendations")):
                                rec_dict["imdb_id"] = fetch_master_imdb_id(app_config, logger, rec_dict["title"], str(rec_dict.get("year","")), False, f"recommendation {rec_dict['title']}")

            logger.info(f"  Finalizing entry for '{movie_title_for_calls}'.")
            try:
                # Ensure all Optional fields that are None are explicitly set if not in working_data_dict
                # to avoid Pydantic treating them as missing for non-Optional fields in sub-models if structure is partially there.
                # For MovieEntry, most complex fields are Optional.
                # Default missing optional top-level fields from LLM3 if LLM3 failed entirely.
                if not llm3_output and active_enrichers_cfg.get('analytical_data'): # if analytical data was supposed to run but failed
                    for fld in LLMCall3Output.model_fields.keys():
                        if fld not in working_data_dict:
                             working_data_dict[fld] = None # Explicitly set to None if LLM3 failed
                ...
                logger.debug(f"  Problematic working_data_dict for '{movie_title_for_calls}': {str(working_data_dict)[:1000]}...")
                if not is_updating_existing_movie :
                    processed_movie_titles_lower_set.add(movie_title_for_calls.lower().strip())
